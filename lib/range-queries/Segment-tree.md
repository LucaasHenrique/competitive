...
1. que informação precisamos guardar em cada node? a quantidade de predios que podem ser destruidos? 
2. O correto a se pensar é: 
    guardar qual a menor altura de um intervalo `[l, r]`;
    em cada query verificamos `h > p`:
        se sim:
            o terremoto nao consegue destruir nenhum predio
        se não:
            temos pelos menos um predio que poder ser destruido.
3. usamos a ideia anterior, porem para cada terremos fazemos varios buscas ate o menor h ser maior que p. 
    isso funciona pq a cada predio destruido, removemos esse predio do array, então é garantido que esse predio só vai ser removido uma unica vez.

```c++
// quantos h <= p existem no intervalo [l, r]? =>  qual o menor h dentro de [l, r]
// se min_h > p, nenhum predio pode ser desruido, se nao algum predio pode ser destruido
...
void solve() {
    ...
    while (q--) {
        ...
        if (t == 1) {
            ...
            ll ans = 0;
            while (true) {
                Node x = seg::query(l, r - 1);

                if (x.mn > p) break;
                ans++;

                seg::update(x.pos, INF64);
            }
